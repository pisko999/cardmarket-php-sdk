...
class OrderChangeStateEntity extends BaseEntity
{
     ...
     * Constructor.
     *
     * @param string $action Action to perform (use STATE_CHANGE_* constants)
     * @param string|null $reason Optional reason for the action
     * @param string|null $relistItems Optional indication if articles should be relisted
     */
    ...
    {
        return '<action>' . htmlspecialchars($this->action, ENT_XML1, 'UTF-8') . '</action>' .
            ($this->reason !== null ? '<reason>' . htmlspecialchars($this->reason, ENT_XML1, 'UTF-8') . '</reason>' : '') .
            ($this->relistItems !== null ? '<relistItems>' . htmlspecialchars($this->relistItems, ENT_XML1, 'UTF-8') . '</relistItems>' : '');
    }
}
